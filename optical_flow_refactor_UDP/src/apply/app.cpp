...
static void wifi_init(void) {
    ...
    IPAddress local_ip(ESP_IP_1, ESP_IP_2, ESP_IP_3, ESP_IP_4);
    IPAddress gateway(GATEWAY_IP_1, GATEWAY_IP_2, GATEWAY_IP_3, GATEWAY_IP_4);
    IPAddress subnet(SUBNET_IP_1, SUBNET_IP_2, SUBNET_IP_3, SUBNET_IP_4);
    WiFi.config(local_ip, gateway, subnet);
#endif
    WiFi.mode(WIFI_STA);
    WiFi.begin(WIFI_SSID_NAME, WIFI_PASSWORD);
    printf("[WiFi] connecting to %s", WIFI_SSID_NAME);
...
void appInit(void){
  ...
  /* WiFi — must come before rtos_init so UDP socket is reachable */
  wifi_init();
